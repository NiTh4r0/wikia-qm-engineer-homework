...
public class AddVideoToHomeworkPageTest {
  ...
  public void testAddVideoLink(String addVideoUrl, String videoUrl, String videoTitle) {
    ...
    assertEquals(flashElementPage.getMessage(),
        "Video page File:".concat(videoTitle).concat(" was successfully added."));

    WikiaFilePage filePage = flashElementPage.clickOnFlashLinkByTitle(videoTitle);
    assertEquals(filePage.getCurrentUrl(), "http://qm-homework.wikia.com/wiki/File:".concat(
        WebElementHelper.convertSpacesToUnderscores(videoTitle)));
  }
